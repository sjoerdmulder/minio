...
func TestRWLockedFile(t *testing.T) {
	...
	rwlk, err := RWLockedOpenFile(f.Name())
	if err != nil {
		t.Fatal(err)
	}
	if rwlk.Size() != 0 {
		t.Fatal("File size should be zero", rwlk.Size())
	}
	isClosed := rwlk.IsClosed()
	if isClosed {
	...
	if err = rwlk.Close(); err != os.ErrInvalid {
		t.Fatal(err)
	}
