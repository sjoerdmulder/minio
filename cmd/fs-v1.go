...
func (fs fsObjects) Shutdown() error {
	...
	return removeAll(preparePath(pathJoin(fs.fsPath, minioMetaTmpBucket, fs.fsUUID)))
}
...
func (fs fsObjects) DeleteBucket(bucket string) error {
	...
	if err = removeAll(minioMetaMultipartBucketDir); err != nil {
		return toObjectErr(err, bucket)
	}

	// Cleanup all the bucket metadata.
	minioMetadataBucketDir := pathJoin(fs.fsPath, minioMetaBucket, bucketMetaPrefix, bucket)
	if err = removeAll(minioMetadataBucketDir); err != nil {
		return toObjectErr(err, bucket)
	}
...
func (fs fsObjects) CopyObject(srcBucket, srcObject, dstBucket, dstObject string, metadata map[string]string) (ObjectInfo, error) {
	bucketDir, err := fs.getBucketDir(srcBucket)
	if err != nil {
	...
	if err != nil {
		return ObjectInfo{}, toObjectErr(traceError(err), srcBucket)
	}
...
func (fs fsObjects) GetObjectInfo(bucket, object string) (ObjectInfo, error) {
	...
	_, err = fsStatDir(bucketDir)
	if err != nil {
		return ObjectInfo{}, toObjectErr(traceError(err), bucket)
	}
...
func (fs fsObjects) DeleteObject(bucket, object string) error {
	...
	if bucket != minioMetaBucket {
		lk, lerr := lock.LockedOpenFile(preparePath(fsMetaPath), os.O_WRONLY, 0600)
		if lerr != nil {
	...
	// Delete the object.
	if err = fsDeleteFile(bucketDir, pathJoin(bucketDir, object)); err != nil {
		return toObjectErr(traceError(err), bucket, object)
	}
