...
func TestFSStats(t *testing.T) {
	...
	for i, testCase := range testCases {
		if testCase.srcPath != "" {
			if _, err := fsStatFile(pathJoin(path, testCase.srcVol, testCase.srcPath)); err != testCase.expectedErr {
				t.Fatalf("TestPosix case %d: Expected: \"%s\", got: \"%s\"", i+1, testCase.expectedErr, err)
			}
		} else {
			if _, err := fsStatDir(pathJoin(path, testCase.srcVol)); err != testCase.expectedErr {
				t.Fatalf("TestPosix case %d: Expected: \"%s\", got: \"%s\"", i+1, testCase.expectedErr, err)
			}
		}
	}
...
func TestFSRemoves(t *testing.T) {
	...
	for i, testCase := range testCases {
		if testCase.srcPath != "" {
			if err = fsRemoveFile(pathJoin(path, testCase.srcVol, testCase.srcPath)); err != testCase.expectedErr {
				t.Errorf("Test case %d: Expected: \"%s\", got: \"%s\"", i+1, testCase.expectedErr, err)
			}
		} else {
			if err = fsRemoveDir(pathJoin(path, testCase.srcVol, testCase.srcPath)); err != testCase.expectedErr {
				t.Error(err)
			}
		}
	}
